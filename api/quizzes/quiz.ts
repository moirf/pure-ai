...
async function loadQuestionsFromDB(): Promise<Question[]> {
  if (!ddbClient || !ddbTable) return normalizedQuestions;
  if (dbQuestionsCache) return dbQuestionsCache;
  try {
    const res = await ddbClient.send(new ScanCommand({ TableName: ddbTable }));
...
async function getQuestionById(id: number): Promise<Question | undefined> {
  if (!ddbClient || !ddbTable) {
    return normalizedQuestions.find((q) => q.id === id);
  }
  try {
    // Use PK/SK schema: pk = `QUESTION#<id>`, sk = `META`
    const pk = `QUESTION#${id}`;
    const res = await ddbClient.send(new GetCommand({ TableName: ddbTable, Key: { pk, sk: 'META' } } as any));
    const it = res.Item;
    if (!it) {
  ...
  try {
    ...
    try {
      if (ddbClient && ddbTable) {
        ...
        await ddbClient.send(new UpdateCommand({ TableName: ddbTable, Key: { pk: questionPk, sk: q.sk ?? String(q.id) }, UpdateExpression: updateExpr, ExpressionAttributeNames: exprNames, ExpressionAttributeValues: exprValues } as any));
      }
    } catch (e) {
...
export const getQuiz = async (event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if (!ddbClient || (!ddbTable && !sessionsTable)) return { statusCode: 400, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'DynamoDB not configured' }) };
...
export const seedQuestions = async (event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    try {
      const scanRes = await ddbClient.send(new ScanCommand({ TableName: ddbTable, ProjectionExpression: 'pk, id' } as any));
    ...
    for (const batch of batches) {
      ...
      let resp = await ddbClient.send(new BatchWriteCommand(params));
      let tries = 0;
      while (resp && resp.UnprocessedItems && Object.keys(resp.UnprocessedItems).length > 0 && tries < 5) {
        ...
        resp = await ddbClient.send(new BatchWriteCommand({ RequestItems: resp.UnprocessedItems }));
        tries++;
      }
