...
describe('Quiz lifecycle (mocked stores)', () => {
  test('create -> start -> get question -> answer -> finish', async () => {
    ...
    const createEv = makeEvent('POST', '/api/quiz', { sessionId: 'S-ABC' });
    ...
    // 5) finish quiz
    const finishEv = makeEvent('POST', '/api/quiz/finish', { quizId, answers: [1], summary: { score: 1 } });
